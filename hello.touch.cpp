...
// static char *message = "Hello World!4"; // NOLINT
static char message[] = "Hello World!4        ";
...
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event) { // NOLINT
  ...
  case SDL_EVENT_MOUSE_MOTION: {
    SDL_snprintf(message, sizeof(message), "Mouse xrel %f", event->motion.xrel);
    break;
  }
  case SDL_EVENT_FINGER_MOTION: {
    SDL_snprintf(message, sizeof(message), "tfinger.dx %f", event->tfinger.dx);
    break;
  }
  }
  return SDL_APP_CONTINUE;
